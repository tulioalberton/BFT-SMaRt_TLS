...
public class TOMConfiguration extends Configuration {
	...
	// spanning tree
	private Boolean spanningTree = false;
	private Boolean treeMultiRooted;
	...
	protected void init() {
		...
		try {
			 ...
			 * Tulio Ribeiro
			 * 
			 * SSL/TLS configuration parameters. Default values: # ssltls = true; #
			 * keyStoreFile = "EC_KeyPair_256.pkcs12"; # enabledCiphers = new String[]
			...
			s = (String) configs.remove("system.treeMultiRooted");
			if (s == null) {
				treeMultiRooted = false;
			} else {
				treeMultiRooted = Boolean.parseBoolean(s);
			}

			s = (String) configs.remove("system.spanningTree");
			if (s == null) {
				spanningTree = false;
			} else {
				spanningTree = Boolean.parseBoolean(s);
			}
	...
	/**
	 * Indicates if signatures should be used true or not false to authenticate
	 * client requests
	 */
	...
	/**
	 * Tulio Ribeiro ## Spanning tree getters and setters
	 */
	public boolean getMultiRooted() {
		return this.treeMultiRooted;
	}

	public boolean isEnabledSpanningTree() {
		return this.spanningTree;
	}

	public void setSpanningTree(boolean isEnabled) {
		this.spanningTree = isEnabled;
	}
