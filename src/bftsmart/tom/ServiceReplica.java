...
public class ServiceReplica {
	 ...
	 * the requests' timer, and drop all enqueued requests, thus letting the
	 * ServiceReplica object be garbage-collected. From the perspective of the rest
	 * of the system, this is equivalent to a simple crash fault.
	 */
	public void kill() {
	 ...
	 * rest of the system, this is equivalent to a rash followed by a recovery.
	 */
	public void restart() {
		Thread t = new Thread() {
					...
					if (null == request.getReqType()) {
						...
						case ORDERED_REQUEST:
							...
							if (executor instanceof BatchExecutable) {
								...
								logger.debug(
										"Delivering request from " + request.getSender() + " via SingleExecutable");
						...
						default: // this code should never be executed
							throw new RuntimeException("Not catched Request Type, " + "should never reach here!");
						}
				} else if (request.getViewID() < SVController.getCurrentViewId()) {
					// message sender had an old view, re-send the message to
