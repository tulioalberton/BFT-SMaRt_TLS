...
public final class ExecutionManager {
	...
	public final boolean checkLimits(ConsensusMessage msg) {
		...
		logger.debug("I'm at consensus " + inExec + " and my last consensus is " + lastConsId);
			...
			if (stopped) {// just an optimization to avoid calling the lock in normal case
				...
				if (isRetrievingState || msg.getNumber() > (lastConsId + 1)
						...
						|| (inExec == -1 && msg.getType() != MessageFactory.PROPOSE)) { // not propose message for the
																						// next consensus
					logger.debug("Message for consensus " + msg.getNumber()
	...
	public void processOutOfContext(Consensus consensus) {
		...
		if (messages != null) {
			logger.debug("[Consensus " + consensus.getId() + "] Processing other " + messages.size()
			...
			for (Iterator<ConsensusMessage> i = messages.iterator(); i.hasNext();) {
				...
				if (consensus.isDecided()) {
					logger.debug("Consensus " + consensus.getId() + " decided.");
					break;
				}
			}
			logger.debug("[Consensus " + consensus.getId() + "] Finished out of context processing");
		}
