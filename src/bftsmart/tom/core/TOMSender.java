...
public abstract class TOMSender implements ReplyReceiver, Closeable, AutoCloseable {
	...
	private void startsCS(int clientId) {
		...
		this.useSignatures = this.viewController.getStaticConf().getUseSignatures();
	...
	public int generateRequestId(TOMMessageType type) {
		lock.lock();
		int id;
		if(type == TOMMessageType.ORDERED_REQUEST || type == TOMMessageType.REQUEST_LEADER)
			id = sequence++;
		else
	...
	public void TOMulticast(byte[] m, int reqId, int operationId, TOMMessageType reqType) {
		...
		switch (reqType) {
		case REQUEST_LEADER:
			System.out.println("Request message to leader...");
